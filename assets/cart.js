...
class CartItemsUpdate extends HTMLElement {
    ...
    // if (this.tagName === 'CART-ITEMS') {
      fetch(`${routes.cart_url}?section_id=mini-cart`)
        ...
        .then((responseText) => {
          ...
          themeSlidersInit($("#cart-upsell-product"));
           ...
           const element = document.querySelector('#mini-cart .free-shipping-wrapper');
            if (element) {
      ...
      fetch(`${routes.cart_url}?section_id=main-cart`)
        ...
        .then((responseText) => {
          ...
          const updatedMainCartSummary = html.querySelector('#main-cart-summary');
            if (updatedMainCartSummary) {
              document.querySelector('#main-cart-summary').innerHTML = updatedMainCartSummary.innerHTML;
            }
        // cartfootertab();
           const element = document.querySelector('#mini-cart .free-shipping-wrapper');
            if (element) {
  ...
  updateLiveRegions(line, message) {
    ...
    if (lineItemError) {
      lineItemError.querySelector('.cart-item-error').innerHTML = message
    };
  }
